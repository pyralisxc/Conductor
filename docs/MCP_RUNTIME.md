...
When a binding supplies `vercelProject`, `deployment.status` reads the provider-native project, current production deployment, latest production attempt, recent deployments, source Git SHA/ref when available, and domains. `deployment.logs` reads bounded/redacted deployment-event output for one exact deployment. `deployment.audit` and `deployment.env.list` return bounded configuration and variable metadata without secret values. Exact redeploy, Git-source deploy, promote, rollback, and environment-variable writes are exposed only with durable mutation state, client write scope, active repository work scope, exact project/deployment or variable identity, and explicit production approval when applicable. Environment values are write-only; receipts and idempotency state do not return or store recoverable secret values. Vercel remains authoritative for deployment state; Conductor does not persist a deployment ledger.

Runtime logs use Vercel's documented `GET /v1/projects/{projectId}/deployments/{deploymentId}/runtime-logs` endpoint. The installed integration has returned `PERMISSION_DENIED` for that endpoint even while deployment and environment metadata reads succeed. Vercel's published integration scope mapping does not list this endpoint under the deployment scope. `preflight_operation` therefore reports runtime-log access as degraded; an exact deployment read is required to establish actual permission. `deployment.logs` covers build/deployment events, not a substitute for runtime logs. Account usage/spend/budget data remain unavailable on this Hobby team; the audit returns the available plan and explicit partial coverage.
...
- Vercel writes are exact-project, idempotent, and gated; production traffic and production-variable changes require exact owner approval. There is no arbitrary deployment deletion, bulk cleanup, alias mutation, or secret-value read tool.
- Runtime logs may be denied by an integration installation even when build logs and deployment reads work. Do not present runtime-log preflight as provider permission proof.
