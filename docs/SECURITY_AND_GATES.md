...
The MCP server requires an authenticated write-scoped client for mutations. For `work-item.create`, an exact repository name is required and the provider must permit issue creation there. Development OS must establish the current routing intent, destination ownership, and suitable visibility; issue routing does not authorize implementation in the destination. Code/development writes independently require the client's primary repository or an explicit temporary extra repository granted by the owner at `/work-scope` using the fingerprint reported by `work-scope.identity`. Temporary code-work scopes expire within 24 hours and can be revoked immediately; expiration restores the default primary repository. The server resolves repository aliases before checking code-work scope and fails closed on storage errors. A chat instruction alone cannot change the server's code-work permissions. Do not send sensitive findings to a destination with broader visibility.
